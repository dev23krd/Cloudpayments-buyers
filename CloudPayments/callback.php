...
$order = $buyers->orders->get_order(intval($_POST['InvoiceId']));
if (empty($order)) {
    exit_with_error(CLOUDPAYMENTS_RESULT_ERROR_INVALID_ORDER);
}

// Получаем метод оплаты
$payment_method = $buyers->payment->get_payment_method($order->payment_method_id);
if (empty($payment_method)) {
    exit_with_error(CLOUDPAYMENTS_RESULT_ERROR_NOT_ACCEPTED);
}
$payment_settings = unserialize($payment_method->settings);
$payment_currency = $buyers->money->get_currency(intval($payment_method->currency_id));
...
$amount = floatval($buyers->money->convert($order->total_price, $payment_method->currency_id, false));
...
if ($is_payment_callback) {
    ...
    $purchases = $buyers->orders->get_purchases(array('order_id' => intval($order->id)));
    foreach ($purchases as $purchase) {
        $variant = $buyers->variants->get_variant(intval($purchase->variant_id));
...
if (($action == 'pay' && $_POST['Status'] == 'Completed') || $action == 'confirm') {
    ...
    $buyers->orders->update_order(intval($order->id), array('paid' => 1));

    // Отправим уведомление на email
    $buyers->notify->email_order_user(intval($order->id));
    $buyers->notify->email_order_admin(intval($order->id));
    ...
    $buyers->orders->update_order(intval($order->id), array('paid' => 0, 'note' => $note));
